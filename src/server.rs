...
impl Handler<Disconnect> for Server {
    ...
    fn handle(&mut self, msg: Disconnect, _ctx: &mut Context<Self>) {
        ...
        info!("after Disconnect - members: {:?}", self.sessions.keys());
    }
}
...
impl Handler<CreateRoom> for Server {
    ...
    fn handle(&mut self, msg: CreateRoom, _ctx: &mut Context<Self>) {
        ...
        if *node_id != self.node_id {
            info!("CreateRoom - Distributing message to node {}", node_id);
            return self
                .net
                .do_send(DistributeMessage(msg.room_id.clone(), msg));
        }

        if let Some(ref mut _room) = self.rooms.get_mut(&msg.room_id) {
            return;
        } else {
            self.rooms.insert(msg.room_id.clone(), HashSet::new());
            info!("CreateRoom - Room {} created", msg.room_id);
        }
    }
}
        ...
        if let Some(ref mut sessions) = self.rooms.get_mut(&msg.room_id) {
            ...
            Response::fut(
                self.net
                    ...
                    .map_err(|_| ())
                    .map(|res| {
                        info!("network members res:{:?}", res);
                        res.unwrap().unwrap_or(Vec::new())
                    }),
            )
        }
    }
